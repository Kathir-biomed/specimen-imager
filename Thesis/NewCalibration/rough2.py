...
marker_dict = cv.aruco.getPredefinedDictionary(cv.aruco.DICT_5X5_100)
...
# Set the coordinate system for the marker (assuming a square of 40mm)
objPoints = np.array([[-20, -20, 0], [20, -20, 0], [20, 20, 0], [-20, 20, 0]], dtype=np.float32)
        ...
        for i, corners in enumerate(marker_corners):
            if len(corners) >= 4:  # Ensure there are enough points for pose estimation
                ...
                rvec, tvec, _ = cv.aruco.estimatePoseSingleMarkers(
                    corners, 40, cameraMatrix, distCoeffs
                )
